...
describe('Shared License Validation', () => {
  ...
  it('should throw error when no license is provided', () => {
    expect(() => validateLicense('MISSING_ENV_VAR', 'pro', testSecret))
      .toThrow('MISSING_ENV_VAR environment variable required');
  });

  it('should throw error for invalid JWT format', () => {
    process.env.TEST_LICENSE = 'invalid-jwt';
    expect(() => validateLicense('TEST_LICENSE', 'pro', testSecret))
      .toThrow('License validation failed: Invalid JWT format');
  });

  it('should throw error for invalid signature', () => {
    // Valid JWT structure but wrong signature
    const invalidJWT = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJwcm9tcHQtY2hhaW5tYWlsLmNvbSIsInRpZXIiOiJwcm8iLCJleHAiOjk5OTk5OTk5OTl9.wrong-signature';
    process.env.TEST_LICENSE = invalidJWT;
    expect(() => validateLicense('TEST_LICENSE', 'pro', testSecret))
      .toThrow('License validation failed: Invalid JWT signature');
  });
  ...
  it('should throw error for wrong issuer', () => {
    ...
    const payload = 'eyJpc3MiOiJ3cm9uZy5jb20iLCJ0aWVyIjoicHJvIiwiZXhwIjo5OTk5OTk5OTk5fQ'; // {"iss":"wrong.com","tier":"pro","exp":9999999999}
    ...
    expect(() => validateLicense('TEST_LICENSE', 'pro', testSecret))
      .toThrow('License validation failed: Invalid license issuer');
  });

  it('should throw error for wrong tier', () => {
    const proJWT = generateTestJWT('pro', testSecret);
    process.env.TEST_LICENSE = proJWT;
    expect(() => validateLicense('TEST_LICENSE', 'enterprise', testSecret))
      .toThrow('License validation failed: Invalid license tier for enterprise package');
  });
  ...
  it('should throw error for expired license', () => {
    ...
    expect(() => validateLicense('TEST_LICENSE', 'pro', testSecret))
      .toThrow('License validation failed: License has expired');
  });
  ...
  it('should validate correct pro license', () => {
    ...
    expect(consoleSpy).toHaveBeenCalledWith(expect.stringContaining('✅ Pro license validated'));
  ...
  it('should validate correct enterprise license', () => {
    ...
    const consoleSpy = vi.spyOn(console, 'log').mockImplementation(() => {});
    expect(validateLicense('TEST_LICENSE', 'enterprise', testSecret)).toBe(true);
