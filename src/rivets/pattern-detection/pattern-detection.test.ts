...
describe("patternDetection(...)", () => {
  ...
  describe("Performance", () => {
    ...
    it("should process large text within performance threshold", async () => {
      const largeText =
        "This is a test message with potential patterns. ".repeat(50);
      const result = await measurePerformance(
    ...
    it("should process 1KB payload within performance threshold", async () => {
      const payload1KB =
        "This is a realistic API payload with various content that might contain injection patterns. ".repeat(
          12
        );
      const result = await measurePerformance(
    ...
    it("should process 10KB payload within performance threshold", async () => {
      const payload10KB =
        "This is a larger API payload simulating document processing, chat history, or complex prompts with potential security risks. ".repeat(
          80
        );
      const result = await measurePerformance(
    ...
    it("should process 100KB payload within performance threshold", async () => {
      const payload100KB =
        "Large document content or extensive chat history that needs security scanning. This simulates real-world enterprise usage. ".repeat(
          800
        );
      const result = await measurePerformance(
